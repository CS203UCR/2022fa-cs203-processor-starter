...
typedef void(*bcd_impl)(char **, int32_t*, uint32_t);
...
//			for(auto & density: densities ) {
				...
				for(auto & function : functions) {
					...
					auto fut = reinterpret_cast<bcd_impl>(f_spec.second);
					...
					if(verify)
					{
						if(function.find("bcd_solution_c") != std::string::npos)
						{
                            int correctness = 1;
							...
							function_spec_t t = function_map::get()[std::string("bcd_reference_c")];
							auto verify_fut = reinterpret_cast<bcd_impl>(t.second);
